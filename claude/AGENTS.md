...
- Keep specs uncommitted on `main` and implementation worktrees under `<project-root>/.worktrees/`.
...
- Treat agents and long commands as event-driven. Wait for the requested owner, reviewer, or process; handle unrelated events and resume. Run long commands in the background and rely on completion notifications. Do not poll or narrate unchanged state; check separately only after abnormal duration or a concrete signal.
