...
- Require `agentrc-codex-code-review` on `PATH`; the agentrc Claude installer provides it.
...
- Re-review through the same runner with `--commit <fix-sha>` or `--uncommitted`, the prior finding identifiers, verification evidence, dispositions, and a request to check fixes and regressions against the original rubric and output contract.
