...
3. **Doc-review** — Invoke `adversarial-doc-review` on the plan/spec. A capable Codex caller uses the fresh-context `doc_reviewer` native subagent; Claude, other harnesses, and Codex without the named role before review start use the managed read-only CLI runner. Both paths use GPT-5.6 Sol at xhigh effort for non-hard reviews and max effort for hard reviews. Native review starts and continues only from a provably read-only parent local sandbox; a writable or unknown capable parent stops for a permission change instead of silently falling back. Address findings before coding.
...
6. **Code-review** — After a development phase and before merging, invoke the shared `code-review` skill. A capable Codex caller uses the fresh-context `code_reviewer` native subagent; Claude, other harnesses, and Codex without the named role before review start use the managed read-only CLI runner. Both paths use GPT-5.6 Sol at xhigh effort for non-hard reviews and max effort for hard reviews. Native review starts and continues only from a provably read-only parent local sandbox; a writable or unknown capable parent stops for a permission change instead of silently falling back. The native ordinary-agent path replaces dedicated `codex exec review` and must pass its seeded-defect validation before equivalence is claimed. For each finding, **verify it's real before acting** (reviewers misread context and hit sandbox artifacts), then fix it or push back with specific reasoning — never silently skip, never blindly implement.
7. **Merge** — Use the `merge` skill.

- **Cap each logical `adversarial-doc-review` or `code-review` workflow at THREE completed substantive reviewer turns** (initial review plus at most two re-reviews). Failed dispatches and transport errors without completed output do not consume a turn, while replacement targets, stateless runner invocations, and transport selection or availability do not reset the count. Reuse the native workflow's canonical target with `followup_task`; never switch an in-progress native workflow to the CLI, and ask the user before a fourth completed turn. Only a materially new scope starts a new workflow, and disclose that reset.
