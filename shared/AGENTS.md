...
3. **Doc-review** — Invoke `adversarial-doc-review` on the plan/spec. It uses a nested read-only Codex CLI reviewer with GPT-5.6 Sol at xhigh effort for non-hard reviews and max effort for hard reviews, shared by Claude and Codex. Address findings before coding.
