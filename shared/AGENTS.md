...
1. **Brainstorm (automatic when necessary)** — Use the `brainstorming` skill for material uncertainty that could change the outcome. It tests the working view against the strongest relevant counterview, asks only consequential questions, converges on an approved design, and does not write the implementation plan.
...
3. **Doc-review** — Invoke `adversarial-doc-review` on the plan/spec. It uses a nested read-only Codex CLI reviewer with GPT-5.6 Sol at high effort for non-hard reviews and max effort for hard reviews, shared by Claude and Codex. Address findings before coding.
