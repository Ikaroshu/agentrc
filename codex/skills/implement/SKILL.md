...
5. Verify every finding using the `code-review` dispositions. Send only confirmed required repairs to the same implementer with `followup_task`; keep accepted non-blocking deferrals as brief residual risks or separately authorized follow-up issues. Require a repair commit and send it to the same reviewer. Continue once the intermediate checkpoint has no confirmed required repairs.
