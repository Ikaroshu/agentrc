...
5. Verify dispositions. Send only confirmed repairs to the same implementer with `followup_task`, require a repair commit, and return it to the same reviewer. Keep accepted deferrals brief or in separately authorized issues. Continue when required repairs are gone.
...
Reuse the reviewer, or start it if none exists, for cumulative integration over the full immutable review-base-to-candidate diff. Supply base, candidate commit and tree, accepted checkpoints, `git diff --stat --find-renames <base>...<candidate>`, `git diff --find-renames <base>...<candidate>`, and `git rev-parse <candidate>^{tree}`; one commit is never sufficient. Route confirmed repairs through the same implementer and repair commit, then rerun cumulative review focused on the repair and plausible interactions without reopening accepted scope. When repairs are gone, run complete repository verification once on the exact clean reviewed commit and record commit, tree, command, and result for `merge`. Any later code change requires a new checkpoint commit, applicable review, cumulative acceptance, and verification.
