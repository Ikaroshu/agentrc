...
3. Snapshot sibling task names. Choose the smallest positive ordinal unused by `code_reviewer_workflow_<N>_stage_*` and reserve `code_reviewer_workflow_<N>_stage_1`.
4. Call `spawn_agent` with the dynamic task as `message`, the exact `agent_type="code_reviewer"`, the reserved `task_name`, `fork_turns="none"`, `model="gpt-5.6-sol"`, and the selected `reasoning_effort`.
5. Retain the returned canonical target. Accepted exact-role dispatch is the role-selection evidence. Do not require unavailable parent-visible child metadata or treat reviewer self-report as proof of its role, model, or effort.
...
Fail loudly for an empty scope, unavailable exact-role dispatch, ambiguous native dispatch, unavailable targets, or unauthorized side-effect attempts. Harness-only failures still require local verification.
