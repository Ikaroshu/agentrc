...
- Use `model="gpt-5.6-sol"` with `reasoning_effort="xhigh"` for bounded established work; use `reasoning_effort="max"` for materially difficult contracts, state, security, irreversibility, blast radius, or unfamiliar architecture.
...
- final: immutable base-to-candidate commands, accepted checkpoints, and `git rev-parse <candidate-sha>^{tree}`

Start each implementation or standalone review with a fresh, uniquely named task using `agent_type="code_reviewer"`, `fork_turns="none"`, and the selected exact model and effort. Retain the implementation reviewer and use `followup_task` for later checkpoints, repairs, and final review. If replaced, give the fresh exact reviewer all accepted scopes and dispositions. Fail loudly if the exact role is unavailable; retry transport failures only with that role.
...
One complete review per requested checkpoint or final candidate is the default. Follow up only for a code-changing repair or unresolved blocker, checking the repair and plausible interactions without reopening unchanged scope. Keep identities and dispositions explicit; when required repairs are gone, report residual risk and advance to implementation or merge approval.
