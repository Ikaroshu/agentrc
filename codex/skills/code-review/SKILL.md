...
When a pass is clear, record the accepted commit and tree and ask for merge approval immediately. Do no implementation or verification work between acceptance and that request. A later code or configuration change or failed check invalidates acceptance without resetting the review budget. If pass two leaves a required repair or material uncertainty, stop without a third review and report the blocker using the global communication guidance.
