...
description: Adversarially review an implementation-ready spec before implementation with the exact native doc_reviewer role, then verify its findings.
---
...
- From the main checkout, require one non-empty regular file with `--spec <path>` and resolve it to an absolute path. Focus text adds emphasis; it never narrows scope.
- Review the outcome, design, and verification as the complete contract. Do not require implementation sequencing that the design itself does not constrain.
...
Treat the supplied spec as one document review unit with a hard budget of two completed review passes, including the initial review. Repairs, replacement reviewers, and follow-up turns do not reset the count. Pass one is the default; advance immediately if it is clear, and use pass two only when a repair materially changes the contract or an unresolved blocker needs confirmation. Reuse the reviewer when practical, otherwise use a fresh exact reviewer. Non-blocking suggestions, documented limitations, and implementation-only questions do not keep review open.
