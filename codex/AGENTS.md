...
- Use the implementer subagent for substantive or complex implementation. Simple changes with a straightforward design and verification may be completed directly by the main agent. A settled design or absence of a spec does not by itself make work simple. Scale verification and review to impact. When user intent is materially uncertain, use brainstorming to settle the goal with the user first, then the design, outcome, and verification. Brainstorming also writes and reviews a spec when a durable contract is needed. Ask before optional workflows.
