...
- Implement a settled change directly in the main agent without an implementer subagent, and scale verification and review to impact. When user intent is materially uncertain, use brainstorming to settle the goal with the user first, then the design, outcome, and verification. Brainstorming also writes and reviews a spec when a durable contract is needed. Ask before optional workflows.
